...
const allowedCodiconImportFiles = new Set(['src/App.vue'])
...
function inspectFile(fullPath) {
  ...
  if (content.includes("@vscode/codicons")) {
    if (!allowedCodiconImportFiles.has(relativePath)) {
      addViolation(relativePath, 'direct @vscode/codicons import is only allowed in src/App.vue')
    }
  }
}
