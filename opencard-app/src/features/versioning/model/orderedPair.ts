...
type IndexedItem<T, Identity> = {
  identity: Identity
  item: T
  index: number
}

function indexItems<T, Identity>(
  items: readonly T[],
  identity: (item: T) => Identity,
): { items: IndexedItem<T, Identity>[]; byIdentity: Map<Identity, IndexedItem<T, Identity>> } | null {
...
export function orderedPair<T, Identity>(
  ...
  for (const rightEntry of indexedRight.items) {
    ...
    pairs.push(matchingLeft
      ? {
        status: 'matched',
        leftItem: matchingLeft.item,
        leftIndex: matchingLeft.index,
