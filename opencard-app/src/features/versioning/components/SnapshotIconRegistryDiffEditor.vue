<template>
  <section class="snapshot-icon-registry-diff-editor">
    <section v-for="side in sides" :key="side.key" class="snapshot-icon-registry-diff-editor__side">
      <header class="snapshot-icon-registry-diff-editor__header">
...
.snapshot-icon-registry-diff-editor__side {
  display: grid;
...
.snapshot-icon-registry-diff-editor__side + .snapshot-icon-registry-diff-editor__side {
  border-left: var(--oc-border-width) solid var(--oc-border-default);
}
.snapshot-icon-registry-diff-editor__header {
  display: flex;
  align-items: center;
  gap: var(--oc-space-2);
...
.snapshot-icon-registry-diff-editor__header strong { color: var(--oc-fg-default); }
</style>
