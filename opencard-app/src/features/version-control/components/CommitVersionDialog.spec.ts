...
describe('CommitVersionDialog', () => {
  it('submits a trimmed commit message', async () => {
    const wrapper = mountDialog()
    await wrapper.get('textarea').setValue('  Update card layout  ')
    await wrapper.get('form').trigger('submit')

    expect(wrapper.emitted('submit')).toEqual([['Update card layout']])
  })
