import { CardDocument, CardComponent, BaseComponent, CSSValue } from '../core/cardDocument'
...
export function componentToHtml(comp: CardComponent): string {
    const baseStyle = getPositionStyle(comp)

    switch (comp.type) {
        case 'text':
            return `<div style="${baseStyle}">${comp.content}</div>`
        case 'image':
            return `<img style="${baseStyle}" src="${comp.assetId}" />`
        default:
            return ''
    }
}

function toCSSValue(value: CSSValue | undefined): string {
    if (value === undefined) return '0px'
    return typeof value === 'number' ? `${value}px` : value
}

export function getPositionStyle(comp: BaseComponent): string {
